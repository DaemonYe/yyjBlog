...
public class CheckcodeController extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		for (int i = 0; i < 4; i++) {
			...
			int degree = new Random().nextInt() % 30; // 产生一个-30-30范围内的数
			 ...
			 * 平移到指定位置，旋转指定弧度，然后向回平移相同的距离 这等同于以下调用序列： translate(x, y);
			 * rotate(theta); translate(-x, -y);
			 */
			graphics2d.rotate(degree * Math.PI / 180, x, 20); // 设置旋转角度

			graphics2d.drawString(ch, x, 20);

			graphics2d.rotate(-degree * Math.PI / 180, x, 20); // 让字转回到0度
			x += 30;
		}
		// 将验证码存入session
		...
		for (int i = 0; i < 5; i++) {
			// 起始点坐标
			int x1 = new Random().nextInt(WIDTH); // 通过随机数发生器设置,nextInt(WIDTH)-->在WIDTH范围内
