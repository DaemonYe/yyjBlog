		...
		<article>
			<div class="container-fluid">
		      <div class="row">
		            ...
		            <table class="table table-striped">
		              ...
		              <tbody>
		              	<c:forEach var="blog" items="${pagedBlogs.result}">
			                  ...
			                  <c:if test="${blog.deleteStatus == '0' }">
			                  	<td>否</td>
			                  </c:if>
			                  <c:if test="${blog.deleteStatus == '1' }">
			                  	<td><strong style="color:red;">是</strong></td>
			                  </c:if>
			                  <td>
			                  	<a href="<c:url value='/admin/updateBlogPage.htm'/>?blogId=${blog.blogId }">修改博文</a>&nbsp;&nbsp;&nbsp;&nbsp;
			                  	<c:if test="${blog.deleteStatus == '0' }"><a href="<c:url value='/admin/deleteBlog.htm'/>?blogId=${blog.blogId }" onclick="javascript:if(confirm('确定要删除此博文？')){return true;}return false;">删除</a></c:if>&nbsp;&nbsp;&nbsp;&nbsp;
			                  	<c:if test="${blog.recommendStatus == '0' }"><a href="<c:url value='/admin/recommendBlog.htm'/>?blogId=${blog.blogId }" onclick="javascript:if(confirm('确定要推荐此博文？')){return true;}return false;">推荐</a></c:if></td>
			                </tr>
		                </c:forEach>
		              </tbody>
		            </table>
		          ...
		          <div class="pagebar">
		          	<!-- 分页显示导航栏 -->
		          	<yyjBlog:PageBar pageUrl="/admin/listBlog.htm" pageAttrKey="pagedBlogs"/>
		          </div>
		        </div>
		      </div>
		    </div>
